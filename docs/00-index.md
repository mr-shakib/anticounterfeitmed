...
The project mark lives in `assets/brand/mark.webp`, carried over from the public
site. Every sized copy — launcher icons, favicons, page headers — is generated by
`make brand` rather than placed by hand, so the app, the portal and the landing
page cannot drift apart when the mark changes.
...
| Label symbol (D21) | **Built** — ordinary scanners read only the public URL; the token rides after the terminator for the app and the portal camera, both reading every codeword with ZXing. One layout, three parsers, 11 shared vectors. **Needs the on-device check in doc 11 before printing.** |
