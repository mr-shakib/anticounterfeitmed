...
| # | Decision | Why it matters | Status |
| --- | --- | --- | --- |
| D9 | **Google Play Console account and Play Integrity setup.** | **PARTLY ANSWERED 2026-09-17: the Play Console account already exists, so the $25 fee is paid and no further spend is required.** Two things still need confirming: whether it is a *personal* account created after 13 Nov 2023, which would impose a 12-tester/14-day closed test before production access (organization accounts are exempt), and creation of the free Firebase project supplying `APP_CHECK_PROJECT_NUMBER` and the app id. **The app side is built and fails closed** (`consumer-app/lib/data/attestation.dart`): a release build with no Firebase project refuses to produce a token, so nothing ships that quietly skips attestation. What remains is the project itself. | **Open — confirm account type** |
...
| --- | --- | --- | --- |
| D17 | **What serves `/` once QRs are printed?** | The root currently serves the project's public marketing page, which has no install instructions and no fragment stripping. Since D21, a current label lands there with no token at all; an original-format label still lands with the token in the URL bar and browser history. Three options in docs/10. **Needs an owner decision before any QR is printed.** | **Open — blocking print** |
| D18 | **Register a short domain for the QR?** | **Superseded 2026-09-22 by D21.** The D21 label format carries the token as 32 raw bytes rather than 43 characters, which already brings the symbol down to version 6 (0.408 mm modules at 20 mm / EC Q) on the current domain. | **Closed** |
| D21 | **Should an ordinary scanner see the token?** | **ANSWERED 2026-09-22 by the owner: no.** An ordinary scanner must read only `https://anticounterfeitmed.com/`; only our app and the portal's print-line camera read the token. **DEVIATION from SRS §2.1**, which prints the token in the fragment and rules out a hidden payload. Implemented in `medcrypto/labels.py` (doc 09). Accepted consequences: a keyboard-wedge scanner can no longer read the token, so print-line scans of current labels use the portal's camera; the token is hidden from casual readers, **not secret** from a raw-byte decoder or a photocopy; copying before first redemption stays unsolved. | **Closed** |
| D19 | **Firebase App Check, Play Integrity direct, or neither?** | **Resolved 2026-09-17: Firebase App Check with the Play Integrity provider.** Costs nothing — App Check is unbilled and Play Integrity is free within a 10,000/day quota — and it verifies offline against a cached JWKS, adding no round trip to a scan. Implemented and tested in `apps/verification/attestation.py`. **Do not attach the reCAPTCHA Enterprise provider**, which bills above 10,000 assessments per month. | **Closed** |
